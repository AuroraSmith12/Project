...
int passward_input(void)
{
    ...
    for(i=0;i<6;)
    {
        ...
        if(keynum_func==15&&mima_flag1)//x修改密码
        {
            //蜂鸣器提示
            				GPIO_ResetBits( GPIOC,  GPIO_Pin_13);
                            delay_ms(1000);
            ...
            keynum_func=MatrixKeyNum_Show();
                ...
                if(keynum_func>0&&keynum_func<10)
                {
                    key[xiugai_num++]=keynum_func;
                    if(xiugai_num==6)
                    {
                        //蜂鸣器提示修改成功
            				GPIO_ResetBits( GPIOC,  GPIO_Pin_13);
...
void task_esp8266(void*param)
{
    for(;;)
    {
        if(GPIO_ReadInputDataBit(GPIOB,GPIO_Pin_11))/* 密码正确 */
        {
						 GPIO_ResetBits( GPIOC,  GPIO_Pin_13);
							Lock_Open();
							OLED_ShowString(2,1,"Successed!");
